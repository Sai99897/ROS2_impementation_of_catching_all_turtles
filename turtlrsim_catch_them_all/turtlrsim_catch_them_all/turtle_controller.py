...
from catch_all_turtle_interfaces.msg import Turtle
from catch_all_turtle_interfaces.msg import TurtleArray
from catch_all_turtle_interfaces.srv import CatchTurtle


class TurtleControllerNode(Node): # MODIFY NAME
    def __init__(self):
        super().__init__("turtle_controller") 
        self.declare_parameter("catch_closest_turtle_fisrt",True)
        self.catch_closest_turtle_first_=self.get_parameter("catch_closest_turtle_fisrt").value
        ...
        self.get_logger().info("velocity input has been started!!!!!")
    ...
    def control_loop(self):
        ...
        if distance>0.5:
            ...
            if diff > math.pi:               # normalize the angle
                diff-=2*math.pi
            elif diff < -math.pi:
                diff+=2*math.pi
            msg.angular.z=6*diff             # calibrated according to P controller
        else:
            msg.linear.x=0.0
            msg.linear.y=0.0
